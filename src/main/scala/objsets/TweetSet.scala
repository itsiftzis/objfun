...
class Empty extends TweetSet {
  ...
  override def mostRetweeted: Tweet = throw new NoSuchElementException("no tweet in empty set");
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    ...
    TweetReader.allTweets.filter( (x:Tweet) => { google.exists( t => x.text.contains(t) ) } )
  }
  lazy val appleTweets: TweetSet = {
    TweetReader.allTweets.filter( (x:Tweet) => { apple.exists( t => x.text.contains(t) ) } )
  }
...
object Main extends App {
  ...
  //GoogleVsApple.googleTweets.foreach( (x:Tweet) => println(x) )
